...
that retarget/parse_mocap.py emits, so everything downstream in animal2go2 --
...
def validate(kp):
    """The checks animal2go2's own contract validator applies."""
    problems = []
    n = int(kp["num_frames"])
...
def main():
    ...
    side.write_text(json.dumps({
        "source": source, "producer": "video2go2/parse_video.py",

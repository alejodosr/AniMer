...
from contacts_2d import lowpass, runs, LEGS
...
def camera_to_world(H_inv, focal, cx, cy):
    ...
    if C[2] < 0:
        raise SystemExit(
            ...
            "X,Y axes given to calibrate_ground.py are left-handed, so world "
            "Z comes out pointing down. Swap two of the clicked points so that "
            "X cross Y points up.")
...
def main():
    ...
    if not focal:
        raise SystemExit("no calibrated focal length: pass --focal or re-run "
                         "calibrate_ground.py with --focal-px")
    focal = float(focal)
    ...
    # animal2go2's own ground_align is built to absorb.
    lifted = 0.0

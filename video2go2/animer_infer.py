...
Lives in the AniMer repo and runs under the AniMer conda env, because it needs
torch + detectron2 + smplx. animal2go2 deliberately has none of those, so this
stage is a subprocess boundary and its output is a plain npz:

    /home/alejodosr/anaconda3/envs/animal/bin/python video2go2/animer_infer.py \
        --video media/dog_1.mov --out <outdir>/dog_1_animer.npz

Everything downstream of it is pure numpy/scipy and needs no GPU. Nothing is
written into animal2go2 yet; that migration happens once this path works.
...
def main():
    ...
    p.add_argument("--checkpoint", default="/media/SHARED_DATA/postcapitalistrobots/"
                   "animer/checkpoints/gdrive_AniMer/checkpoints/checkpoint.ckpt",
                   help="the 8.35 GB vith checkpoint; the 2.7 GB one declares "
                        "BACKBONE.TYPE=vit and the loader rejects it")

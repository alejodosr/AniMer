...
LEGS = ["FR", "FL", "RR", "RL"]
MIN_SEGMENT_S = 0.05     # matches animal2go2 postprocess.MIN_SEGMENT_S
...
def refine_contacts(contacts, min_len):
    ...
    Same semantics and same order as animal2go2's postprocess.refine_contacts:
...
def main():
    ...
    p.add_argument("--metres-per-unit", type=float, default=1.133,
                   help="metres per SMAL unit; from check_phaseb.py, which "
                        "fits it to a breed-typical shoulder height")
